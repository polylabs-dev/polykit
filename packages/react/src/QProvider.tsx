/**
 * QProvider — app context provider
 *
 * Loads qkit.wasm, initializes SPARK identity, and provides
...
interface QProviderProps {
  /** URL to the app's .wasm file (includes qkit + app-specific circuits) */
  wasm: string;
  /** HKDF derivation context (e.g., "q-data-v1") */
  hkdfContext: string;
  /** App identifier (e.g., "polydata") */
  appId?: string;
  /** Lex namespace (e.g., "polyqlabs.data") */
  lexNamespace?: string;
...
export function QProvider({
  ...
  if (state.error) {
    return <div className="qkit-error">QKit initialization failed: {state.error}</div>;
  }

  if (!state.ready) {
    return <div className="qkit-loading">Loading QKit...</div>;
  }
