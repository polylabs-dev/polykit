...
### FLIR WASM Client Build Pipeline (#550)
- The `estream-dev build-wasm-client` single-command build is excellent DX — one command from FLIR to signed WASM
...
**Codegen impact**: The FLIR compiler resolves profiles at parse time before codegen. The Rust generator sees fully-expanded annotations — zero codegen changes needed. Profile resolution is a preprocessing step:
...
**Codegen impact**: The FLIR compiler emits budget metadata into the `.escd` manifest:
...
```rust
// Auto-generated by FLIR codegen when @budget.wasm < 64KB
#[cfg(feature = "compact")]
...
When `@meters` is declared, the FLIR → Rust codegen wraps every exported function with metering instrumentation:
...
pub fn upload_file(ctx: &mut Context, payload: &[u8]) -> Result<()> {
    // AUTO-GENERATED by FLIR codegen from @meters annotation
    ctx.metering.record(Dimension::Bandwidth, payload.len() as u64);
    ctx.metering.record(Dimension::Circuits, 1);
    // --- end auto-generated ---
...
The FLIR → Rust codegen wraps every `emit()` call with sanitization:
...
```
error[FLIR-S001]: unsanitized emission in @sanitize circuit
...
The FLIR compiler resolves `composes:` into a dependency graph, validates version constraints, and generates Rust imports:
...
The FLIR → Rust codegen generates typed subscription handler stubs and emission helpers:
...
Currently, FLIR `types:` sections define structs and enums but the codegen only produces bare Rust structs. Useful trait implementations should be auto-generated:
